...
- **`--anthropic`**: requires `~/.claude/.credentials.json` on the host (populated by `claude login`). The real OAuth `accessToken` is embedded directly into the mitm rules; the pod gets a fake `.credentials.json` (`sk-ant-oat01-agent-uplink-*` tokens, `expiresAt` pinned ~10 years out) so the Claude CLI takes the OAuth code path and shows the welcome banner. There is no fallback auth path; if the credentials file is missing or unparseable, startup fails.
...
The host `settings.json` is copied wholesale. `claude_settings_bytes` copies it into the pod as-is: it drops only the top-level `sandbox` key, replaces `permissions`, and merges the mode's injected placeholder into `env`. It applies no allow-list, so secret-bearing keys — `apiKeyHelper` and any secret `env` entries (e.g. API tokens) — in the host `settings.json` reach the agent container's `~/.claude/settings.json`. Keep secrets out of the host `settings.json`. The same applies to the managed settings below.
...
- `permissions` is replaced with `defaultMode: auto` (plus `skipDangerousModePermissionPrompt`), exactly as for the user settings.
  Otherwise a host policy's `permissions` would override the `auto` mode set on `~/.claude/settings.json` and the agent would
  prompt inside the sandbox;
...
- the auth mode's placeholder `env` (e.g. `AWS_BEARER_TOKEN_BEDROCK=placeholder`) is merged in and wins over the host's value.
  Otherwise a real credential in the host policy's `env` would reach the agent container and defeat the placeholder shipped in
  `settings.json`.
